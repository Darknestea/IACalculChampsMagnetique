...
def train(create_model_function, experiment_name, user_id, session, use_real_images, test_size=0.2, random_state=42):
    ...
    history = model.fit(
        x_train, y_train, epochs=50, batch_size=32, validation_data=(x_test, y_test), callbacks=[early_callback]
    )
    ...
    evaluate_model(model, model_information[HISTORY_FIELD], x_test, y_test, number_samples=50, record_images=True)
