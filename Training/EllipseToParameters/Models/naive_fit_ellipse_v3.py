...
def remove_bad_points(points):
    ...
    median_dist = np.quantile([np.linalg.norm(point) for point in centered_points], 0.5)
    return np.array([point for point, centered_point in zip(points, centered_points) if median_dist*0.8 <= np.linalg.norm(centered_point) <= median_dist*1.3])
